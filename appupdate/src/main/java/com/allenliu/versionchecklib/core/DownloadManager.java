...
public class DownloadManager {
    ...
    public static void downloadAPK(final String url, final VersionParams versionParams, final DownloadListener listener) {
        ...
        String downloadPath = versionParams.getDownloadAPKPath() + getFileName(url);
        //静默下载也判断本地是否有缓存
        ...
        AllenHttp.getHttpClient().newCall(request).enqueue(new FileCallBack(versionParams.getDownloadAPKPath(), getFileName(url)) {
            @Override

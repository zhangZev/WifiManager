...
public class UIActivity extends AllenBaseActivity implements DialogInterface.OnCancelListener {
    ...
    private void dealVersionDialogCommit() {
        ...
        if (versionBuilder != null) {
            if(!VersionService.isAlive()){
                VersionService.enqueueWork(getApplicationContext());
            }
            //如果是静默下载直接安装
            if (versionBuilder.isSilentDownload()) {
                String downloadPath = versionBuilder.getDownloadAPKPath() + DownloadManager.getFileName(versionBuilder.getDownloadUrl());
            ...
            if(versionBuilder.getForceUpdateListener() == null){
                finish();
            }
